...
jQuery(document).ready(function ($) {
    ...
    if(adsenseinewsletter.do_tour){
        var content = '<h3>Thanks for using WP ADSENSEI!</h3>';
        content += '<p>Do you want the latest on <b>WP ADSENSEI update</b> before others and some best resources on monetization in a single email? - Free just for users of WP ADSENSEI!</p>';
        ...
        content += '<form class="ml-block-form" id="adsensei-subscribe-newsletter-form" method="POST" target="_blank">';
        ...
        content += '<input type="text" name="name" class="form-control" placeholder="Name" hidden value="'+adsenseinewsletter.current_user_name+'" style="display:none">';
        content += '<input type="text" value="'+adsenseinewsletter.current_user_email+'" name="email" class="form-control" placeholder="Email*"  style="      width: 180px;    padding: 6px 5px;">';
        content += '<input type="text" name="company" class="form-control" placeholder="Website" hidden style=" display:none; width: 168px; padding: 6px 5px;" value="'+adsenseinewsletter.path+'">';
        ...
        wp_pointers_tour_opts = $.extend (wp_pointers_tour_opts, {
            ...
            close: function () {
                $.post (ajaxurl, {
                    pointer: 'wpadsensei_subscribe_pointer',
        ...
        setup = function () {
            $("#toplevel_page_adsensei-settings").pointer(wp_pointers_tour_opts).pointer('open');
        };
    ...
    $("#adsensei-subscribe-newsletter-form").on('submit',function(e){
        e.preventDefault();
        var $form = $("#adsensei-subscribe-newsletter-form");
        var name = $form.find('input[name="name"]').val();
        var email = $form.find('input[name="email"]').val();
        var website = $form.find('input[name="company"]').val();
           $.ajax({
      url:adsensei_localize_data.rest_url + "apiAdsensei/adsensei_subscribe_newsletter",
      type:"POST",
      headers: { 
