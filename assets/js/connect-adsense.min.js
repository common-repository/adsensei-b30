;(function($){
    $( document ).on( 'change', '#report_type,#report_period,#input_based,#report_view_type,#cust_fromdate,#cust_todate', function(){
        ...
        if(report_type == 'earning_forcast')
            var url =  adsensei_localize_data.rest_url + 'adsensei-adsense/get_report_adsense';
        else
            var url =  adsensei_localize_data.rest_url + 'adsensei-adsense/get_report_adsense';
        ...
        if($.trim(report_type) !='' ) {
            if(!adsensei_localize_data.is_pro && report_type!='earning'){
                display_report('');

            }else if($.trim(report_period) !=''){
                $('#adsensei_reports_canvas').html('<div class="adsensei-cover-spin"></div>');
                setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (response, status, XHR) {
                            ...
                            if (response) {
                                ...
                                $('#adsensei_reports_canvas').html('<h5> Please select Report type and Duration</h5>');
                            }
                        },
                        error: function (request, status, error) {
                            $('#adsensei_reports_canvas').html('<h5> Please select other Date</h5>');
                            console.log('error');
                        },
                    });
                }, 200);
    ...
    $( document ).on( 'click', '#adsensei-connect-adsense', function(){
        if ( $( this ).hasClass( 'disabled_adsense_link' ) ) return;
        if ( 'undefined' != typeof window.adsenseiMapiConnect ) {
            window.adsenseiMapiConnect();
        }
    } );

    // Unique instance of "adsenseiMapiConnectClass"
    var INSTANCE = null;

    var adsenseiMapiConnectClass = function( content, options ) {
        this.options = {};
    ...
    adsenseiMapiConnectClass.prototype = {

        constructor: adsenseiMapiConnectClass,
        ...
        init: function(){
            ...
            $( document ).on( 'click', '#adsensei-mapi-confirm-code', function(){
                ...
                var data = {
                    action: 'adsensei_gadsense_mapi_confirm_code',
                    code: code,
            ...
            $( document ).on( 'click', '.gadsense-modal-content-inner[data-content="account-selector"] button', function( ev ) {
                ...
                if ( details && JSON ) {
                    $( '.gadsense-overlay' ).css( 'display', 'block' );
                    var data = {
                        action: 'adsensei_gadsense_mapi_select_account',
                        nonce: adsensei.nonce,
        ...
        show: function( content ) {
            ...
            if ( 'confirm-code' == content ) {
                ...
                this.AUTH_WINDOW = window.open( adsensei_adsense.auth_url, 'adsenseiOAuth2', args );
        ...
        getAccountDetails: function( tokenData ){
            var data = {
                action: 'adsensei_gadsense_mapi_get_details',
                nonce: adsensei.nonce,
            };
                ...
                success:function(response, status, XHR){
                    if ( response.status && true === response.status ) {
                        ...
                        if ( response['raw']['errors'][0]['message'] ) {
                            ...
                            if ( 'undefined' != typeof adsensei.connectErrorMsg[response['raw']['errors'][0]['reason']] ) {
                                INSTANCE.frame.find( '.error-description' ).html( adsensei.connectErrorMsg[response['raw']['errors'][0]['reason']] );
                            } else {
    ...
    window.adsenseiMapiConnectClass = adsenseiMapiConnectClass;

    // Shortcut function.
    window.adsenseiMapiConnect = function( content, options ) {
        if ( 'undefined' == typeof content || ! content ) {
            content = 'confirm-code';
        }
        if ( 'undefined' == typeof options ) {
        ...
        if ( null === INSTANCE ) {
            INSTANCE = new adsenseiMapiConnectClass( content, options );
        } else {
...
function display_report(response){
    ...
    if(report_type == 'top_device_type') {
        if(adsensei_localize_data.is_pro) {
        ...
        if(!adsensei_localize_data.is_pro && report_type == 'earning_forcast') {
            ...
            for (index = 0; index < data_length; ++index) {
                if (report_view_type == 'week') {
                    ...
                    var week_number = adsensei_get_week_number(date);
        ...
        document.getElementById('adsensei_report_table').innerHTML = dynamic_tablet;
...
function adsensei_get_week_number(date){
