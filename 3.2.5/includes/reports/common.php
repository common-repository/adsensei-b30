...
if( !defined( 'ABSPATH' ) )
	exit;
add_action( 'admin_enqueue_scripts', 'adsensei_load_adsnese_scripts', 100 );
...
add_action( 'rest_api_init', 'adsensei_registerRoute');

function adsensei_registerRoute($hook){
	register_rest_route( 'adsensei-adsense', 'adsensei_confirm_code', array(
		'methods'    => 'POST',
		'callback'   => 'adsensei_confirm_code',
		'permission_callback' => function(){
			return current_user_can( 'manage_options' );
		}
	));
	register_rest_route( 'adsensei-adsense', 'adsensei_adsense_get_details', array(
		'methods'    => 'POST',
		'callback'   => 'adsensei_adsense_get_details',
		'permission_callback' => function(){
			return current_user_can( 'manage_options' );
		}
	));
	register_rest_route( 'adsensei-adsense', 'get_report_status', array(
		'methods'    => 'POST',
		'callback'   => 'adsensei_adsense_get_report_status',
		'permission_callback' => function(){
			return current_user_can( 'manage_options' );
		}
	));
	register_rest_route( 'adsensei-adsense', 'get_report_adsense', array(
		'methods'    => 'POST',
		'callback'   => 'adsensei_adsense_get_report_data',
		'permission_callback' => function(){
			return current_user_can( 'manage_options' );
		}
	));
	register_rest_route( 'adsensei-adsense', 'get_report_stats', array(
		'methods'    => 'POST',
		'callback'   => 'adsensei_ads_stats_get_report_data',
		'permission_callback' => function(){
			return current_user_can( 'manage_options' );
		}
	));
	register_rest_route( 'adsensei-adsense', 'get_report_abtesting', array(
		'methods'    => 'POST',
		'callback'   => 'adsensei_adsense_get_report_abtesting_data',
		'permission_callback' => function(){
			return current_user_can( 'manage_options' );
		}
	));
	register_rest_route( 'adsensei-adsense', 'revoke_adsense_link', array(
		'methods'    => 'POST',
		'callback'   => 'adsensei_adsense_revoke_adsense_link',
		'permission_callback' => function(){
			return current_user_can( 'manage_options' );
		}
	));
}
function adsensei_adsense_revoke_adsense_link($request_data){
	$parameters = $request_data->get_params();
	$options = adsensei_get_option_adsense();
	...
	if ( is_wp_error( $response ) ) {
		...
		delete_option("adsensei_adsense_api_data");
		echo json_encode( array( 'status' => true ) );
	}
}
function adsensei_adsense_get_report_status($request_data){
	$parameters = $request_data->get_params();
	$reportlist =array();
	$adsensei_get_option_adsense =adsensei_get_option_adsense();
	$account_id ='';
	$status = false;
	if(isset($adsensei_get_option_adsense['accounts']) && !empty($adsensei_get_option_adsense['accounts'])){
		//$account_id =implode(',',array_keys($adsensei_get_option_adsense['accounts']));
		$account_id = $adsensei_get_option_adsense['accounts'][""]["details"];
		$status = true;
	}
	$reportlist['adsense'] =  array("status"=>$status,"account_id" =>$account_id);
	return $reportlist;
}
function adsensei_confirm_code($request_data){
...
function adsensei_adsense_get_details($request_data){
	$parameters = $request_data->get_params();
	...
	if ( ! is_array( $token_data ) ) {
		...
		echo json_encode(
			array(
				'status'    => false,
				'error_msg' => esc_html__( 'No token provided. Token data needed to get account details.', 'adsenseib30' ),
			)
		);
		die;
	...
	if ( is_wp_error( $response ) ) {
		...
		if ( isset( $accounts ) ) {
			...
			update_option( 'adsensei_adsense_api_data', $options );
			...
			// adsensei_save_token_from_data( $token_data, $accounts['items'][0]);
			adsensei_save_token_from_data( $token_data, $accounts["accounts"][0]);
			echo json_encode(
				array(
					'status'     => true,
			...
			if ( isset( $accounts['error'] ) ) {
				...
				$options = get_option('adsensei_adsense_api_data');
				$options['connect_error'] = array(
					'message' => $msg,
				);
...
function adsensei_get_option_adsense(){
	...
	$options = get_option( 'adsensei_adsense_api_data', array() );
	if ( ! is_array( $options ) ) {
		$options = array();
	}
...
function adsensei_save_token_from_data( $token, $details, $args = array() ) {
...
function adsensei_adsense_get_report_abtesting_data(){
	global $wpdb;
	$results = $wpdb->get_results( "SELECT * FROM `wp_adsensei_stats` ");
	if(!empty($results)) {    
    $adsensei_table = "<table id=\"blocked_id_table\">"; 
    $adsensei_table.= "<tbody>";
	$adsensei_table.= '<tr class="b_in_" style="font-weight: bold;">
    ...
    $adsensei_table.= "</tbody>";
    $adsensei_table.= "</table>"; 
}
echo json_encode(
	array(
		'status'    => 'success',
		'success_msg' => $adsensei_table,
	));
}

function adsensei_adsense_get_report_data($request_data){
	...
	$token_data    = adsensei_adsense_get_access_token($account_id);
...
function adsensei_ads_stats_get_report_data($request_data, $ad_id=''){
	...
	$array_top5=array();
		...
		if( $day == "last_7_days" ){
			...
			$to_date = $dates_i[0];
			if($ad_id=='all') {
				$results_impresn_S = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_impression, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_date BETWEEN %s AND %s",
						$to_date,
						$from_date
					)
				);
			...
			else
			{
				$results_impresn_S = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_impression, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_date BETWEEN %s AND %s AND `ad_id` = %d",
						$to_date,
						$from_date,
						$ad_id
					)
		...
		$individual_impr_day_counts = $ad_imprsn_values;
			...
			if($ad_id=='all') {
				$results_click_S = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_click, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_date BETWEEN %s AND %s",
						$to_date,
						$from_date
					)
				);
			...
			else
			{
				$results_click_S = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_click, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_date BETWEEN %s AND %s AND `ad_id` = %d",
						$to_date,
						$from_date,
						$ad_id
					)
			...
			if($ad_id=="all")
			{
				$results_top5 = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT `{$wpdb->prefix}posts`.ID, `{$wpdb->prefix}posts`.post_title, 
						SUM(`{$wpdb->prefix}adsensei_single_stats_`.date_impression) as total_impression,
						SUM(`{$wpdb->prefix}adsensei_single_stats_`.date_click) as total_click 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						INNER JOIN `{$wpdb->prefix}posts` 
						ON `{$wpdb->prefix}posts`.ID = `{$wpdb->prefix}adsensei_single_stats_`.ad_id 
						WHERE `{$wpdb->prefix}adsensei_single_stats_`.ad_date BETWEEN %s AND %s
						GROUP BY `{$wpdb->prefix}posts`.post_title 
						ORDER BY SUM(`{$wpdb->prefix}adsensei_single_stats_`.date_click) DESC 
						LIMIT 5",
						$to_date,
						$from_date
					)
			...
			if($ad_id=='all') {
				$results_impresn_F = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_impression, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_date BETWEEN %s AND %s",
						$first_date_,
						$current_date_month_
		...
		if($ad_id=='all') {
			$results_click_F = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT date_click, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_date BETWEEN %s AND %s",
					$first_date_,
					$current_date_month_
		...
		if($ad_id=="all")
			...
			if($ad_id=='all') {
				$results_impresn_F = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_impression, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE MONTH(ad_date) = MONTH(NOW()) - 1 AND YEAR(ad_date) = %d",
						$year
					)
				);
			...
			else
			{
				$results_impresn_F = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_impression, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE MONTH(ad_date) = MONTH(NOW()) - 1 AND YEAR(ad_date) = %d AND `ad_id` = %d",
						$year,
						$ad_id
					)
				);
		...
		if($ad_id=='all') {
			$results_click_F = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT date_click, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE MONTH(ad_date) = MONTH(NOW()) - 1 AND YEAR(ad_date) = %d",
					$year
				)
			);
		...
		else
		{
			$results_click_F = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT date_click, ad_date FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE MONTH(ad_date) = MONTH(NOW()) - 1 AND YEAR(ad_date) = %d AND `ad_id` = %d",
					$year,
					$ad_id
				)
			);
		...
		if($ad_id=="all")
			{
				$results_top5 = $wpdb->get_results(
					$wpdb->prepare(
						...
						INNER JOIN `{$wpdb->prefix}posts` 
						ON `{$wpdb->prefix}posts`.ID = `{$wpdb->prefix}adsensei_single_stats_`.ad_id 
						WHERE MONTH(`{$wpdb->prefix}adsensei_single_stats_`.ad_date) = MONTH(NOW()) - 1 
						AND YEAR(`{$wpdb->prefix}adsensei_single_stats_`.ad_date) = %d 
						GROUP BY `{$wpdb->prefix}posts`.post_title 
						ORDER BY SUM(`{$wpdb->prefix}adsensei_single_stats_`.date_click) DESC 
						LIMIT 5",
						$year
			...
			if($ad_id=="all"){
				$results_impresn_F = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression, ad_year FROM `{$wpdb->prefix}adsensei_single_stats_` GROUP BY ad_year;"
					)
				);
			...
			else
			{
				$results_impresn_F = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression, ad_year FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_id = %d GROUP BY ad_year;",
						$ad_id
					)
				);
			...
			if($ad_id=="all"){
				$results_click_F = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_click) AS date_click, ad_year FROM `{$wpdb->prefix}adsensei_single_stats_` GROUP BY ad_year;"
					)
				);
				...
				$results_click_F = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_click) AS date_click, ad_year FROM `{$wpdb->prefix}adsensei_single_stats_` WHERE ad_id = %d GROUP BY ad_year;",
						$ad_id
					)
				);
		...
		foreach ($array_c as $key => $value) {
			...
			if($ad_id=="all")
			{
				$jan = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 1 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$feb = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 2 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$mar = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 3 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$apr = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 4 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$may = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 5 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$jun = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 6 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$jul = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 7 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$aug = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 8 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$sep = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 9 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$oct = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 10 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$nov = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 11 AND YEAR(ad_date) = %d",
						$year
					)
				);
				...
				$dec = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 12 AND YEAR(ad_date) = %d",
						$year
					)
				);
			}
			...
			{
				$jan = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 1 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$feb = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 2 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$mar = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 3 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$apr = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 4 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$may = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 5 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$jun = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 6 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$jul = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 7 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$aug = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 8 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$sep = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 9 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$oct = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 10 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$nov = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 11 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
				
				$dec = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_impression) AS date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 12 AND YEAR(ad_date) = %d AND ad_id = %d",
						$year,
						$ad_id
					)
				);
			...
			if($ad_id=="all")
			{
				$jan = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT SUM(date_click) AS date_click 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE MONTH(ad_date) = 1 AND YEAR(ad_date) = %d",
						$year
					)
			...
			if($ad_id=="all")
			{
				$results_top5 = $wpdb->get_results(
					$wpdb->prepare(
						...
						INNER JOIN `{$wpdb->prefix}posts` 
						ON `{$wpdb->prefix}posts`.ID = `{$wpdb->prefix}adsensei_single_stats_`.ad_id 
						WHERE YEAR(`{$wpdb->prefix}adsensei_single_stats_`.ad_date) = %d 
						GROUP BY `{$wpdb->prefix}posts`.post_title 
		...
		if($ad_id=="all")
		{
			$results_impresn = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT date_impression 
					FROM `{$wpdb->prefix}adsensei_single_stats_` 
					WHERE `ad_date` = %s",
					$yesterday_date
				)
		...
		if($ad_id=="all")
		{	
			$results_click = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT date_click 
					FROM `{$wpdb->prefix}adsensei_single_stats_` 
					WHERE `ad_date` = %s",
		...
		if($ad_id=="all")
			{
				$results_top5 = $wpdb->get_results(
					$wpdb->prepare(
						...
						INNER JOIN `{$wpdb->prefix}posts`
						ON `{$wpdb->prefix}posts`.ID = `{$wpdb->prefix}adsensei_single_stats_`.ad_id
						WHERE `{$wpdb->prefix}adsensei_single_stats_`.`ad_date` = %s
						GROUP BY `{$wpdb->prefix}posts`.post_title
						ORDER BY total_click DESC
						LIMIT 5",
						$yesterday_date
		...
		if($ad_id=="all")
			{
				$results_impresn_t = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
						WHERE `ad_date` = %s",
		...
		if($ad_id=="all")
		{
			$results_click_t = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT date_click 
					FROM `{$wpdb->prefix}adsensei_single_stats_` 
					WHERE `ad_date` = %s",
		...
		if($ad_id=="all")
			{
				...
				$results_top5 = $wpdb->get_results(
					$wpdb->prepare(
						...
						WHERE `{$wpdb->prefix}adsensei_single_stats_`.`ad_date` = %s 
						GROUP BY `{$wpdb->prefix}posts`.post_title 
						ORDER BY `{$wpdb->prefix}adsensei_single_stats_`.date_click DESC 
						LIMIT 5",
						$todays_date
					)
				);				
		...
		if($ad_id=="all")
			...
			{
				$results_impresn_C_ = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT date_impression 
						FROM `{$wpdb->prefix}adsensei_single_stats_` 
...
function adsensei_adsense_get_access_token($account){
	$options = adsensei_get_option_adsense();
	...
	// 	if ( time() > $options['accounts'][ $account ]['expires'] ) {
		if ( isset( $options['accounts'][""] ) ) {
			...
			$new_tokens = adsensei_adsense_renew_access_token( $account );
		...
		if ( ! empty( $options['accounts'] ) ) {
			...
			return array(
				'status' => false,
				'msg' => esc_html__( 'It seems that some changes have been made in the Adsensei Ads settings. Please refresh this page.', 'advanced-ads' ),
				'reload' => true,
			);
		} else {
			// No account at all.
...
function adsensei_adsense_renew_access_token( $account ) {
...
function adsensei_load_adsnese_scripts($hook){
	if($hook!=='toplevel_page_adsensei-settings'){ return ; }
	...
	wp_enqueue_script( 'adsensei-admin-adsense', $js_dir . 'connect-adsense' . $suffix . '.js', array('jquery'), ADSENSEI_VERSION, false );
	...
	wp_localize_script( 'adsensei-admin-adsense', 'adsensei_adsense', array(
		'auth_url' => $auth_url
	) );

	// ab testing js
	wp_enqueue_script( 'adsensei-admin-abtesting', $js_dir . 'abtesting-reports' . $suffix . '.js', array('jquery'), ADSENSEI_VERSION, false );
	// chart js
	wp_enqueue_script( 'adsensei_charts_js', $js_dir . 'Chart' . $suffix . '.js', array('jquery'), ADSENSEI_VERSION, false );
	//    wp_localize_script( 'adsensei-charts-js' ,'');
	wp_enqueue_script( 'adsensei_charts_js' );
}
 ...
 * @return type string
 */
function adsensei_get_date($type) {
 ...
 * @return type array
 */
function adsensei_get_ad_stats($condition, $ad_id='', $date=null,$parameters ='') {
...
// echo 'sdf';die;	
		$result = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT SUM(`ad_clicks`) as `clicks`, SUM(`ad_impressions`) as `impressions` 
				FROM `{$wpdb->prefix}adsensei_stats` 
				WHERE `ad_id` = %d;",
				$ad_id
			),
			ARRAY_A
        ...
        case 'fetchAllBy':
            ...
            if($ad_id){
                ...
                $result = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT * FROM `{$wpdb->prefix}adsensei_stats` 
						WHERE `ad_thetime` = %d AND `ad_id` = %d;",
						$date,
						$ad_id
					),
					ARRAY_A
			...
			case 'search':
				...
				if($parameters){
					if(isset($parameters['report_period'])){
						...
						$ad_thetime = $wpdb->prepare( 'WHERE ad_thetime BETWEEN %s AND %s', $startDate, $endDate );
					}
				}
				$search_param = '';
				if(isset($parameters['search_param']) && !empty($parameters['search_param'])){
					if(empty($ad_thetime)){
						$search_param = $wpdb->prepare(
							"WHERE ad_id LIKE %s OR
							ad_device_name LIKE %s OR
				...
				$results = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT * FROM `{$wpdb->prefix}adsensei_stats` %s %s LIMIT %d, %d",
						$ad_thetime,
						$search_param,
						$offset,
						$items_per_page
					...
					$result_total = $wpdb->get_row(
						$wpdb->prepare(
							"SELECT count(*) as total FROM `{$wpdb->prefix}adsensei_stats` %s %s",
							$ad_thetime,
							$search_param
						),
						ARRAY_A
