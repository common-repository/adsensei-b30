...
class adsenseiLogger {
  ...
  const FATAL = 0;
	...
	function __construct($file, $level)
	{
		$this->file = $file;
		$this->level = $level;
		$this->path = ADSENSEI_PLUGIN_DIR . "logs/$this->file";
                $this->folder = ADSENSEI_PLUGIN_DIR . "logs";
		$this->start();
	}
	...
	private function log($string)
	{
          ...
          $enabled = isset($adsensei_options['debug_mode']) ? $adsensei_options['debug_mode'] : false;
          if ($enabled)
