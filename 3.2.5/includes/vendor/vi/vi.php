...
class vi {
    ...
    public function __construct() {
        ...
        $this->settings = get_option('adsensei_vi_settings');

        $this->ads = get_option('adsensei_vi_ads');
    }
    ...
    public function hooks() {
        ...
        if (!empty($this->token)) {
          ...
          //  add_action('adsensei_weekly_event', array($this, 'setSettings'));
            // add_action('adsensei_daily_event', array($this, 'setActive'));
            //add_action('adsensei_daily_event', array($this, 'setRevenue'));
           // add_action('adsensei_weekly_event', array($this, 'verifyViAdCode'));
        }
    ...
    public function getDebugNotice() {
        if ($this->debug) {
            echo '<div class="notice notice-error" id="wpadsensei-adblock-notice" style="">ATTENTION: WP ADSENSEI vi debug mode is activated</div>';
        }
        return false;
    }
    ...
    public function registerSettings() {
        register_setting('adsensei_settings', 'adsensei_vi_ads');
    }
    ...
    public function setSettings() {
        ...
        if (is_wp_error($response)) {
            update_option('adsensei_vi_api_error', $response->get_error_message() );
            return false;
        } else {
            delete_option('adsensei_vi_api_error' );
            $response = json_decode($response['body']);
        }


        if (isset($response->status) && $response->status == 'ok' && !empty($response)) {
            update_option('adsensei_vi_settings', $response);
            return true;
        }
        return false;
    }
    ...
    public function getSettings() {
        return get_option('adsensei_vi_settings');
    }
    ...
    public function createAdsTxt() {
        ...
        $adsTxt = new \wpadsensei\adsTxt($vi, '41b5eef6');
    ...
    public function getAdsTxtContent() {
        $vi_token = get_option('adsensei_vi_token');
        if (!$vi_token)
            return false;
    ...
    private function getToken() {
        $token = get_option('adsensei_vi_token', '');
    ...
    public function setAds() {
        update_option('adsensei_vi_ads', $this->ads);
    }
    ...
    public function setRevenue() {
        ...
        // else
        //return $response->data;
        update_option('adsensei_vi_revenue', $response->data);
        return true;
    }
    ...
    public function getRevenue() {
        return get_option('adsensei_vi_revenue');
    }
    ...
    public function setAdCode() {
        ...
        $ads = get_option('adsensei_vi_ads');
...
//            );
        ...
        //return $response->data;
        update_option('adsensei_vi_ads', $ads);
    ...
    public function getAdCode() {
        return get_option('adsensei_vi_ads');
    }
    ...
    public function setActive() {
        $url = 'https://adsplugin.net/vi.html';
        $args = array(
            'method' => 'GET',
            'timeout' => 15,
            'headers' => array(
        ...
        if (is_wp_error($response)) {
            delete_option('adsensei_vi_active');
            return true;
        }
        if (wp_remote_retrieve_response_code($response) == '404' || wp_remote_retrieve_response_code($response) == '401') {
            delete_option('adsensei_vi_active');
        ...
        if ($response['body'] == 'false') {
            update_option('adsensei_vi_active', 'false');
            return false;
        }
    }
    ...
    public function verifyViAdCode() {
        ...
        //$url = 'https://adsplugin.net/wpadsensei-api/signup/create.php?domain='.$this->getDomain().'&hash=' . $this->getHash();
        $url = 'https://adsplugin.net/wpadsensei-api/signup/create.php';
        $args = array(
            'method' => 'POST',
            'timeout' => 15,
            'headers' => array(
    ...
    private function getHash() {
        $string = get_option('adsensei_vi_ads');
