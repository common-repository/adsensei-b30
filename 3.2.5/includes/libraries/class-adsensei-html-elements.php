...
/**
 * ADSENSEI_HTML_Elements Class
 *
 * @since 0.0.9
 */
class ADSENSEI_HTML_Elements {
	...
	public function select( $args = array() ) {
		...
		if( $args['chosen'] ) {
			$args['class'] .= ' adsensei-select-chosen';
		}
		...
		$output = '<select name="' . esc_attr( $args['name'] ) . '" id="' . esc_attr( sanitize_key( str_replace( '-', '_', $args['id'] ) ) ) . '" class="adsensei-select ' . esc_attr( $args['class'] ) . '"' . $multiple . ' data-placeholder="' . $placeholder . '">';
	...
	public function text( $args = array() ) {
		...
		$output = '<span id="adsensei-' . sanitize_key( $args['name'] ) . '-wrap">';

			$output .= '<label class="adsensei-label" for="' . sanitize_key( $args['id'] ) . '">' . esc_html( $args['label'] ) . '</label>';

			if ( ! empty( $args['desc'] ) ) {
				$output .= '<span class="adsensei-description">' . esc_html( $args['desc'] ) . '</span>';
			}
	...
	public function date_field( $args = array() ) {
		...
		if( empty( $args['class'] ) ) {
			$args['class'] = 'adsensei_datepicker';
		} elseif( ! strpos( $args['class'], 'adsensei_datepicker' ) ) {
			$args['class'] .= ' adsensei_datepicker';
		}
	...
	public function textarea( $args = array() ) {
		...
		$output = '<span id="adsensei-' . sanitize_key( $args['name'] ) . '-wrap">';
			...
			$output .= '<label class="adsensei-label" for="adsensei-' . sanitize_key( $args['name'] ) . '">' . esc_html( $args['label'] ) . '</label>';
	...
	public function month_dropdown( $name = 'month', $selected = 0 ) {
		...
		while ( $month <= 12 ) {
			$options[ absint( $month ) ] = adsensei_month_num_to_name( $month );
			$month++;
		}
