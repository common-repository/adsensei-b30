...
if ( window.jQuery ) {
   var adsensei_widget_fixed =   jQuery('.adsensei_widget_fixed').length;
    if ( adsensei_widget_fixed ) {
        jQuery(window).load(adsensei_widget_fixed_init);
    } else {
        if (document.readyState!='loading'){
            adsensei_widget_fixed_init();
        }
        else{
            document.addEventListener('DOMContentLoaded', adsensei_widget_fixed_init);
        }
    }
} else {
    console.log('jQuery is not loaded!');
}
function adsensei_widget_fixed_init(){
    var adsensei_widget_fixed =   jQuery('.adsensei_widget_fixed');
    for ( var i = 0; i < adsensei_widget_fixed.length; i++ ) {
        adsensei_sidebar(adsensei_widget_fixed[i]);
    }
...
function adsensei_sidebar(options){
    var options_current_id = jQuery(options).attr('id');
    var options_current_class ='adsensei_widget_fixed';
    ...
    jQuery('.adsensei-widget-clone-' + options_current_id).remove();

    function widget() {}
	var widgets = new Array();
    var adsensei_widget_fixed =   jQuery('.adsensei_widget_fixed');
    for ( var i = 0; i < adsensei_widget_fixed.length; i++ ) {
      var  widget_obj = jQuery( adsensei_widget_fixed[i]).parent();
        ...
        widget_obj.css('position',''); 
            ...
            widgets[i].clone.addClass('adsensei-widget-clone-' + options_current_id);
                ...
                for ( var i = widgets.length - 1; i >= 0; i-- ) {
                    if (widgets[i]) {
                        ...
                        if ( !widget_parent_container ) {
                            ...
                            widget_parent_container.addClass('adsensei-fixed-widget-container');
