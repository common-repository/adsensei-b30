...
function sanitize_post_meta($key, $meta){
  ...
  function adsenseiGetPostIdByMetaKeyValue($meta_key, $meta_value){
 ...
 * }
 */
function adsensei_validate_ads_txt_line( $line, $line_number ) {
...
function adsensei_change_mode() {
	...
	if($adsensei_settings_backup){
		update_option('adsensei_settings', $adsensei_settings_backup);
	}else{
		update_option('adsensei_settings', $adsensei_settings);
	}
    
	update_option('adsensei_settings_backup', $adsensei_settings);
	update_option('adsensei-mode',sanitize_text_field($_REQUEST['mode']));
	
	wp_send_json ( array('status' => 't') );

}
add_action('wp_ajax_adsensei_change_mode', 'adsensei_change_mode');
///